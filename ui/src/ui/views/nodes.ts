    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          角色：${roleValue} · 权限范围：${scopesValue} · 请求于 ${age}${repair}
        </div>
      </div>
      <div class="list-meta">
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? "已吊销" : "有效";
      ...
      ${state.formMode === "raw"
        ? html`
            <div class="callout warn" style="margin-top: 12px">
              请先把 Config 切到 <strong>表单</strong> 模式，再在这里编辑绑定。
            </div>
          `
        : nothing}
      ${!state.ready
        ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">请先加载配置，再编辑绑定。</div>
            ...
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                ...
                <div class="list-meta">
                  ...
                  ${!supportsBinding
                    ? html` <div class="muted">没有可用 system.run 的节点。</div> `
                    : nothing}
                </div>
              </div>

              ${state.agents.length === 0
                ? html` <div class="muted">未找到 agent。</div> `
